...
function cms_urkunde_out($turnier, $data, $vorlagen, $type) {
	...
	// Spielername
		$line['verein'] = cms_urkunde_zeile_anpassen($line['verein'], 52, 48);
		$abstand_links = 95;
		...
		if (strlen($line['spieler']) > 34 AND !empty($line['vorname'])) {
			...
			$line['spieler'] = cms_urkunde_zeile_anpassen($line['spieler'], 32, 28);
	...
	// Fuß
		...
		$pdf->text(185, 760, $turnier['urkunde_unterschrift1']); 
		$pdf->text(310, 760, $turnier['urkunde_unterschrift2']); 
	}
	return $pdf;
}
